...
public class IntelligentParsingService {
    ...
    private String buildPrompt() {
        ZoneId zone = PromptDateTimeHelper.safeZone(calendarTimezone);
        String dateContext = PromptDateTimeHelper.contextBlock(zone);
        return dateContext
                + """
            ...
            - Use AUTHORITATIVE DATE/TIME CONTEXT above as the only reference for "today", current year, and resolving "next week" / relative phrases.
            - Extract dates from ANY format: "October 15, 2026", "Oct 15", "10/15/2026", "10-15-26", "15th October", "Due: Oct 15", "Deadline: 10/15", etc.
            - Convert ALL dates to ISO 8601 calendar form: YYYY-MM-DD
            - If only month/day is given with no year, assume the current calendar year from the context block above (not a fixed year).
            - If only day is given, infer month from surrounding syllabus text.
            - Look for keywords: "due", "deadline", "by", "on", "before", "until"
            - Handle relative dates: "next week", "in 2 weeks", "end of semester" (convert to concrete YYYY-MM-DD using the context dates).
